  ...
  <div class="quota-wrapper">
    ...
    <el-dialog v-model="dialogVisible" @closed="cleanTempQuota" class="dialog">
      ...
      <div class="quota-no-display">
        <el-checkbox v-model="quotaDisplayValue" label="展示配额剩余数量"> </el-checkbox>
        <el-tooltip
          class="tooltip"
          effect="dark"
          placement="right"
...
const quotaDisplayValue = ref(moduleConfig.value.quotaDisplay)
...
const confirm = () => {
  ...
  emit(FORM_CHANGE_EVENT_KEY, {
    ...
    quotaDisplay: quotaDisplayValue.value
  })
}
...
watch(
  ...
  (val) => {
    ...
    quotaDisplayValue.value = val.quotaDisplay
  },
