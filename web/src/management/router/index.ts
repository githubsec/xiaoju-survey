...
async function handleLoginGuard(
  ...
  if (token) {
    try {
      // Decode username from token (Payload is the second part)
      // Fix Base64URL to Base64 for atob
...
async function handlePermissionsGuard(
  ...
  if (!to.meta.permissions || !currSurveyId) {
    ...
    if (currSurveyId !== prevSurveyId) {
      ...
      // 如果获取不到权限信息（可能是所有者），暂时放行
      if (cooperPermissions.length === 0 || hasRequiredPermissions(to.meta.permissions as string[], cooperPermissions)) {
        next()
      } else {
